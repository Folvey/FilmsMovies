...
public class JSONUtils {
    ...
    public static String BASE_POSTER_URL = "https://image.tmdb.org/t/p/";
    public static final String SMALL_POSTER_SIZE = "w185";
    public static final String BIG_POSTER_SIZE = "w780";

    public static ArrayList<Movie> getMoviesFromJSON(JSONObject jsonObject) {
        ArrayList<Movie> result = new ArrayList<>();
        Log.i("dbg", "getMoviesFromJSON start");
        if (jsonObject == null) {
            Log.i("dbg", "getMoviesFromJSON OBJECT NULL");
            return result;
        }
        try {
            ...
            for (int i = 0; i < jsonArray.length(); i++) {
                ...
                String posterPath = BASE_POSTER_URL + SMALL_POSTER_SIZE +
        ...
        Log.i("dbg", "getMoviesFromJSON over");
        return result;
    }

    public static ArrayList<Review> getReviewsFromJSON(JSONObject jsonObject) {
        ArrayList<Review> result = new ArrayList<>();
        Log.i("dbg", "getReviewsFromJSON start");
        if (jsonObject == null) {
            Log.i("dbg", "getReviewsFromJSON OBJECT NULL");
            return result;
        }
        try {
            JSONArray jsonArray = jsonObject.getJSONArray(KEY_RESULTS);
    ...
    public static ArrayList<Trailer> getTrailerFromJSON(JSONObject jsonObject) {
        ...
        Log.i("dbg", "getTrailersFromJSON start");
        if (jsonObject == null) {
            Log.i("dbg", "getTrailersFromJSON OBJECT NULL");
            return result;
        }
        try {
            JSONArray jsonArray = jsonObject.getJSONArray(KEY_RESULTS);
            for (int i = 0; i < jsonArray.length(); i++) {
                ...
                String videoKey = BASE_YOUTUBE_URL + objectTrailer.getString(KEY_OF_VIDEO);
                String name = objectTrailer.getString(KEY_NAME);
                Trailer trailer = new Trailer(videoKey, name);
                result.add(trailer);
            }
        } catch (JSONException e) {
            e.printStackTrace();
